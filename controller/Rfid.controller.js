...
sap.ui.define([
    ...
    function (Controller, JSONModel, BarcodeScanner, ResourceModel, MessageToast, MessageBox,
        ...
        return Controller.extend("production.controller.Rfid", {
            formatter: formatter,
            oFragments: {},

            onInit: function () {
                var oRouter = this.getOwnerComponent().getRouter();
                oRouter.getRoute("RouteRfid").attachMatched(this._onHandleRouteMatched, this);
            ...
            onChangeMatnrRFID: function (oEvent) {
                ...
                if (oEvent.getSource().getValue() != "") {
                    ...
                    //var oRFIDLine = oEvent.getSource().getBindingContext().getProperty(sPath);
            ...
            onAceptarRFID: function (oEvent) {
                ...
                if (aTable.length > 0) {
                    ...
                    //     var sSelect = aTable[i];
                    // };
                    // var sPath = sSelect + "/Cantidad"
                    // var sRFIDCont = oEvent.getSource().getBindingContext().getProperty(sPath, sPath);
                    // this.getView().getModel("WoutJSON").setProperty('/RFIDCont', sRFIDCont);
                };
            ...
            onValueHelpAufnr: function (oEvent) {
                ...
                that.getFragment("ProductionOrDialog").then(function (oFragment) {
                    ...
                    //                     let tableModel = oTable.getModel("columns");
                    //                     let dateCol = tableModel.getProperty('/cols')[1];
                    //                     let gstrs = oEvent.getParameters().data.results[0].gstrs
                    //                     let formatGstrs = that.formatter.formatDate(gstrs);

                    //                     dateCol.template = formatGstrs;
                    //                     oTable.setModel(tableModel, "columns");
            ...
            handleEnabledSaveBtn: function () {
                ...
                let palletType = viewModel.getProperty('/PalletType')[0].packnrT;
                let rfid = viewModel.getProperty('/RFIDTable')[0].Rfid;
            ...
            cleanScreen: function () {
                ...
                let oModelEditView = this.getView().getModel('ViewJSON'); // Cambio 27/09
                let initialEditView = this.getEditView(); // Cambio 27/09
            ...
            postSave: function () {
                ...
                var sSRfid = this.getSRfid();
                var oWoutJSON = this.getView().getModel("WoutJSON").getObject("/");
                var oViewJSON = this.getView().getModel('ViewJSON'); // Cambio 27/09
                ...
                if (oWoutJSON.Quantity == 0) {//Cammbio 26/09
                    MessageBox.error(errorMessageCancelOper);//Cammbio 26/09
                ...
                if (oWoutJSON.RFIDTable[0].Rfid === '') {//Cammbio 26/09
                    // No carga la tabla //Cammbio 26/09
                    MessageBox.error(errorMessageRfid);//Cammbio 26/09
                    return
                } else {
                ...
                this.getView().getModel("ZDOM_0000_SRV_01").create('/HeaderSet', oPostProduct, {
                    success: function (oData, oResponse) {
                        ...
                        if (oData.ReturnSet.results.length === 0) {
                            errorMessageUnexpected = oResourceBundle.getText("textMessageUnexpected");
                            MessageBox.error(errorMessageUnexpected);
                            busyDialog4.close();
                            that.getView().getModel('ViewJSON').setProperty('/InputProductionOrder', true);
                            that.getView().getModel('ViewJSON').setProperty('/InputMaterial', true);
                            // that.cleanScreen(); // Cambio 27/09
                            // oEditView.setData(oInitEditView); // Cambio 27/09
                            return
                        }

                        if (oData.ReturnSet.results.length > 1) {
                            errorMessageUnexpected = oResourceBundle.getText("textMessageUnexpected");
